-- Fix admin user setup by creating proper Supabase Auth user and linking to admin_users table

-- First, let's ensure we have the admin_users record ready for linking
-- Update the existing admin_users record to be ready for proper user_id linking
UPDATE admin_users 
SET user_id = NULL, 
    username = 'admin',
...
-- Create a more permissive policy for admin creation during setup
DROP POLICY IF EXISTS "Allow initial admin creation when no admins exist" ON admin_users;

CREATE POLICY "Allow admin creation and linking during setup" 
